    ...
    ERC165
{
    ...
    function _validateAccountPrepayment(
        ...
        unchecked {
            ...
            if (paymaster == address(0)) {
                emit RequiredPreFund(requiredPrefund);
    ...
    event RequiredPreFund(uint256 requiredPrefund);
    /**
    ...
    // Utility function to convert address to string for error messages
    ...
    function _validateAccountAndPaymasterValidationData(
        ...
        if (expectedAggregator != aggregator) {
            string memory errorMessage = string(
                abi.encodePacked(
                    "AA24 signature error: expected ",
                    _addressToString(expectedAggregator),
                    ", got ",
                    _addressToString(aggregator)
                )
            );
            //revert FailedOp(opIndex, 'AA24 signature error');
            revert FailedOp(opIndex, errorMessage);
        }
        if (outOfTimeRange) {
                ...
                if (context.length > 0) {
                    ...
                    if (mode != IPaymaster.PostOpMode.postOpReverted) {
                        try
                            ...
                            }(mode, context, actualGasCost, gasPrice) // solhint-disable-next-line no-empty-blocks
                        {} catch {
